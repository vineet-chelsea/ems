...
if ($IncludePythonPackages) {
    ...
    if (Test-Path "$repoRoot\requirements_modbus.txt") {
        # Create temp file without 'struct' (it's built-in)
        $tempReq = Join-Path $env:TEMP "requirements_temp.txt"
        Get-Content "$repoRoot\requirements_modbus.txt" | Where-Object { $_ -notmatch '^\s*struct\s*$' } | Set-Content $tempReq
        pip download -r $tempReq -d $pythonWheelsDir 2>&1 | Out-Null
