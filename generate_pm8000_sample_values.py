...
def get_sample_value(parameter, data_type):
    ...
    if data_type == "FLOAT32":
        if "frequency" in param_lower:
            ...
            elif "crest" in param_lower or "factor" in param_lower:
                return encode_float32(1.5)
            else:
        ...
        elif "k-factor" in param_lower or "k factor" in param_lower:
            return encode_float32(1.5)
        else:
        ...
        elif "disturbance" in param_lower:
            return encode_int32u(0)
        else:
            return encode_int32u(50000)
    
    elif data_type == "INT16U":
        # Counter parameters (dips, swells, interruptions)
        if "dip" in param_lower or "swell" in param_lower:
            return [0]  # Start counters at 0
        elif "interruption" in param_lower:
            return [0]
        elif "phases" in param_lower:
            return [3]
            ...
            return [1]
        elif "harmonic" in param_lower or "hrm" in param_lower:
            return [100]
        else:
            return [100]
        ...
        if "name" in param_lower:
            return encode_utf8("PM8000 Meter")
        elif "model" in param_lower:
            return encode_utf8("PM8000")
...
# Read the JSON file
with open('backend/src/config/pm8000_mappings.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

# Open output file for writing
output_file = 'pm8000_sample_values.txt'
